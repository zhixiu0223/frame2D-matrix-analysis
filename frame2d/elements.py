...
def fixed_end_forces_distributed_moment(m, L):
    ...
    彎矩」的正負號沒有統一標準, 這裡刻意沿用本模組既有的集中力矩慣例,
    圖個內部一致, 不是宣稱這是唯一正確的規定)之固定端反力。

    推導(虛功法, 用標準Hermite形狀函數): 均佈彎矩m對虛轉角場做虛功,
    對應的廣義力是"位移形狀函數對x的導數"(不是位移形狀函數本身,
    這是跟分布力/集中力矩案例的關鍵差異)——F_i = m * integral(dN_i/
    dx dx) = m * (N_i(L) - N_i(0)), 用sympy符號積分驗證過(見對話
    紀錄)得到: Fy1=-m, M1=0, Fy2=m, M2=0(均佈彎矩對固定-固定樑不會
    直接貢獻端點彎矩反力, 全部反力以剪力形式呈現——一開始看到這個
    結果會覺得反直覺, 但用整體力平衡驗證過是自洽的: Fy1+Fy2=0(合力
    為0, 均佈彎矩本身不產生淨側向力), M1+M2+Fy2*L = 0+0+m*L = m*L
    (合力矩剛好等於總施加彎矩m*L), 兩個檢查都通過)。

    這個公式沒有獨立於推導方法本身的"基本物理事實"可以直接核對(不像
    fixed_end_forces_point_moment()當初可以用懸臂樑自由端的個案驗證),
    改用兩種靜定結構的獨立驗證: 簡支樑(兩端反力應該精確是∓m, 純靜力
    平衡就能算出來, 不依賴這條公式)、懸臂樑(固定端彎矩反力應該精確是
    -m*L, 一樣是純靜力平衡)——見tests/test_distributed_moment.py,
    兩者都用實際跑過這個公式組裝出的完整結構分析, 拿反力去對照獨立
    算出來的靜定解, 不是自己驗證自己。
    """
