...
| D11 | 網頁匯出PDF: 模態+反應譜+循環+非線性地震(全部四種) | webapi/pdf_export.py | ✅ 已完成 |
...
#### D10 網頁匯出 Markdown: 模態/反應譜/循環/非線性地震 ✅ 已完成
...
#### D11 網頁匯出 PDF: 全部四種(模態 + 反應譜 + 循環 + 非線性地震) ✅ 已完成
...
**追加: 反應譜PDF匯出**
- `build_rsa_pdf_report()`: 反應譜曲線圖(疊模態點, 紅點+編號標籤)+ 模態組合表(T/f/Sa/Γ/
  有效質量比/累積質量比/模態基底剪力, 標題含總基底剪力跟累積質量比)+ 節點位移/桿件端點內力表
  + 質量設定頁 + 完整輸入資料頁
- 驗證(`tests/test_pdf_export_rsa.py`): 曲線座標點與模態紅點精確對照 `rsa_to_dict()` 的
  curve/modes 資料(含Sa單位換算); 結果表逐格比對(節點位移表逐格核對ux**跟**uy兩欄——這裡
  也踩到跟模態PDF同類型的坑: 第一版只檢查ux欄位, 一個「uy欄位誤用ux值」的突變因此沒被抓到,
  補上uy的逐格檢查才抓到); 固定7頁(曲線1+結果表2+質量1+輸入資料3)。突變檢查4個, 一開始
  3個抓到、1個沒抓到(上述uy的問題), 補強後4/4
- 也發現並修正 `test_pdf_export_modal.py` 遺留的過時檢查: 那個檔案原本把
  `analysis_type='rsa'` 也算進「還在做」的清單裡, 反應譜PDF做完之後這個假設不再成立, 導致
...
**追加: 非線性地震PDF匯出(四種分析類型全部做完)**
- `build_seismic_pdf_report()`: 地震時程圖(地面加速度+位移)+ 全域遲滯圖(位移 vs 有效慣性力
  合力)+ 能量平衡圖(動能/阻尼耗能/桿件內力作功疊加, 對照外力作功虛線, 兩者應該精確蓋在一起
  ——這張圖本身就是驗證這組結果最直接的方式)+ 摘要表(含能量平衡兩個數字的自我檢核, 應該
  精確相等)+ 降伏塑鉸表 + 塑鉸M-θp小圖(**重用D11c已經寫好的 `_cyclic_hinge_fig()`**,
  `seismic_to_dict()`跟`cyclic_to_dict()`的塑鉸dict形狀相容, 不用另外寫一份)+ 質量設定頁 +
  完整輸入資料頁
- **一個版面上的問題**(不是數據錯誤, 純粹排版): 摘要頁一開始把控制節點/方向/阻尼比/週期/
  Rayleigh係數/步數/尖峰位移/能量平衡全部塞進 `_table_page()` 的多行標題, 跟下面第一個
  表格的子標題疊在一起看不清楚。改成把這些摘要資訊拆成獨立的「Summary」表格(每一列一個
  項目, 兩欄: 項目/數值), 版面乾淨很多, 也讓後面測試容易逐格核對(比從一大串標題文字裡
  用正規表示式挖數字穩健)
- 驗證(`tests/test_pdf_export_seismic.py`): 三張圖(時程/遲滯/能量)座標點精確比對
  `seismic_to_dict()` 的資料(含單位換算), 用 `warnings.simplefilter('error')` 確認不觸發
  任何警告(D11c循環PDF踩過字型缺字的坑, 這裡直接照抄同一個防護, 這次沒有再踩到, 因為
  `_hinge_label_en()` 已經是共用的); 用monkeypatch確認塑鉸圖真的呼叫了
  `_cyclic_hinge_fig()`(不是自己重寫一份); 摘要表逐格比對, **用刻意構造「外力作功≠
  KE+阻尼+內力作功」的假資料**測試兩個欄位是不是真的來自不同的來源——正常案例下這兩個值
  本來就該相等(能量平衡成立), 用真實資料測不出「表格第二列不小心又抄了一次外力作功」這種
  錯誤, 只有故意讓兩者不同才測得出來, 這是這一系列PDF匯出測試裡第三次遇到「正常情況下兩個
  該相等的值讓突變測試失效」這類問題(第一次是D11b的uy誤用ux因為兩者剛好都算對, 這次是
  刻意設計不對等的假資料來解決同一類問題)。突變檢查5個, 一開始4個抓到、1個沒抓到
  (上述能量平衡欄位問題), 補上假資料測試後5/5
- **前端一個真實bug, 也是這次順便發現並修正的**: 循環(D11c)跟非線性地震(D6b)的網頁端到端
  測試裡, 各自有一段「沒有塑鉸容量」的錯誤路徑測試, 都用同一種寫法**原地**修改
  `model.members` 的物件屬性來製造錯誤情境, 測完再用JSON備份「還原」`model.members`。但
  `lastCyclicPayload`/`lastSeismicPayload`(匯出PDF/Markdown用的那份存檔資料)透過物件參照
  指到同一批桿件物件, 原地修改會連存檔資料一起弄壞, 「還原」只是把 `model.members` 換成
  新陣列, 沒有動到存檔裡那些已經被改壞的舊物件。這個bug其實在D11c(循環)測試時就抓到並修好
  了一次(見上面的記錄), 但寫非線性地震的PDF匯出測試時發現D6b的`seismic_e2e.js`裡也有一模
  一樣的寫法、還沒修——這次一併修正(改用`.map()`產生新物件, 不動原本的物件, 兩個檔案現在
  都用同一種安全寫法)
- 四種分析類型(模態/反應譜/循環/非線性地震)的PDF匯出到這裡全部完成。四個獨立的
  `test_pdf_export_*.py` 加起來、四個網頁端到端測試(`tests/web/*_e2e.js`)都各自驗證過
  匯出Markdown+匯出PDF兩種格式
