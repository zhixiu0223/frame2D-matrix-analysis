...
層4 網頁後端(`/export/pdf`, analysis_type='modal'): stdlib 與 FastAPI 兩後端萃取出來的
    ...
    裡嵌了產生時間戳記, 兩次呼叫的bytes本來就不會相同, 這不是bug); analysis_type='rsa'/
    'cyclic'/'seismic' 回應清楚的「還在做」訊息, 不是壞掉的PDF或不明錯誤。這一層需要
...
try:
    ...
    for at, keyword in [("rsa", "反應譜"), ("cyclic", "循環"), ("seismic", "非線性地震")]:
    ...
    print("  rsa/cyclic/seismic 的 PDF 匯出回應清楚的「還在做」訊息(不是壞掉的PDF)")
